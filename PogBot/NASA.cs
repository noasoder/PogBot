...
namespace PogBot
{
    public class NASA
    {
        public static async Task<(string, string, string)> GetAPOD()
        {
            ...
            var imageUrl = Global.StringBetween(result, "\"url\":\"", "\"");
            var title = Global.StringBetween(result, "\"title\":\"", "\"");
            return (title, explanation, imageUrl);
        }
    }
}
